...
btnScan.addEventListener("click", async () => {
  ...
  try {
    ...
    console.error(e);
    mostrarAlerta("Erro ao iniciar scanner", "erro");
  }
});
...
async function iniciarScanner() {
  ...
  try {
    ...
    mostrarAlerta("Permissão da câmera negada", "erro");
    return;
  }
  ...
  Quagga.init({
    ...
    if (err) {
      console.error(err);
      mostrarAlerta("Erro ao iniciar câmera", "erro");
      return;
    }
...
function pararScanner() {
  console.log("Parando scanner seguro...");

  scannerAtivo = false;

  try {
    // remove evento primeiro (evita erro depois)
    if (quaggaInicializado) {
      Quagga.offDetected(onDetect);
    }

    // para Quagga só se iniciou
    if (quaggaInicializado) {
      Quagga.stop();
      quaggaInicializado = false;
    }

    // para câmera manualmente
    if (container) {
      ...
      // limpa container depois de parar tudo
      setTimeout(() => {
        container.innerHTML = "";
      }, 100);
    }
...
function onDetect(data) {
  ...
  log("Código bruto: " + bruto);
  log("NCE: " + nce);

  if (nce === ultimoCodigo && (agora - ultimoTempo) < 2500) {
    log("Bloqueado duplicado");
    return;
  }
  ...
  log("Produtos carregados: " + produtos.length);
